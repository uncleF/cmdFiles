...
:projectRename
	...
	for %%a in (.) do set oldProjectName=%%~na
	...
	move .\%oldProjectName%.sublime-project .\%newProjectName%.sublime-project >nul 2>nul
	call uprreplace %newProjectName% %oldProjectName% %newPackageName% %oldPackageName%
	cd ..
	move %oldProjectName% .\%newProjectName% >nul 2>nul
	cd %newProjectName%
	goto exit
